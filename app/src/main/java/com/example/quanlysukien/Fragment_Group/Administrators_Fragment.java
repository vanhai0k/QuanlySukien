...
import com.example.quanlysukien.adapter.AdminGroupAdapter;
...
public class Administrators_Fragment extends Fragment {

    List<UserMember> list;
    AdminGroupAdapter adapter;
    RecyclerView recyclerviewmember;
    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        // Inflate the layout for this fragment
        View view = inflater.inflate(R.layout.fragment_administrators_, container, false);
    ...
    private void callAPI() {
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(Request.Method.GET,
                URL.url + "getAdmingroup?_id=" +idgroup, null, new Response.Listener<JSONObject>() {
            @Override
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        JSONObject adminObject = jsonObject.getJSONObject("admin");
                         UserMember member = new UserMember();
                            ...
                            member.setUsername(adminObject.getString("username"));
                            member.setImage(adminObject.getString("image"));
                            member.setId(adminObject.getString("_id"));

                            list.add(member);
                    }
                    adapter = new AdminGroupAdapter(getContext(),list);
